...
resource "aws_launch_template" "k8-nodes" {
  name = "k8-nodes"
    ...
    device_name = "/dev/xvda"  # Root device for EKS nodes
    
    ebs {
      volume_size           = 150        # Size in GB (increase as needed)
      volume_type           = "gp3"      # gp3 is newer, more cost-effective
...
resource "aws_iam_policy" "nlb_k8s_policy" {
  name        = "${var.tag_prefix}-nlb-k8s-policy"
